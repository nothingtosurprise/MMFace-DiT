...
class UnifiedDualStreamDiT(nn.Module):
    ...
    def forward(self, x, t, text_pooled_embeddings, modality, clip_seq_embeddings=None):
        ...
        # For Rectified Flow Matching, the time t should be scaled from [0, 1] to [0, 999]
        # to be compatible with the t_embedder which expects values in the [0, 999] range
        t_input = t.clone()

        # If the maximum time is 1.0 or less, we are in RFM mode.
        # Scale the time from the [0, 1] range to the [0, 999] range.
        if t_input.max() <= 1.0:
            t_input = t_input * 999.0
        
        t_emb = self.t_embedder(t_input)  # Time embedding: [B, D]
