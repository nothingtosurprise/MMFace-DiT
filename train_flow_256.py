# train_256_unified_rfm.py
# Training script for Unified Dual-Stream DiT with Rectified Flow Matching using either mask or sketch conditioning.
...
# Uses Rectified Flow Matching paradigm where training samples endpoints x0 (noise) and x1 (data) and interpolates with continuous time t.
...
def parse_args():
    ...
    parser.add_argument(
        "--config_path",
        type=str,
        default="config_256_unified.yml",
...
def main():
    ...
    # --- 9. No noise scheduler needed for Rectified Flow Matching ---
    # RFM uses continuous time [0, 1] without a discrete scheduler
    ...
    # Rectified Flow Matching doesn't use SNR weighting, so skip this
    logger.info("Rectified Flow Matching training - not using SNR weighting.")
        ...
        for step, batch in enumerate(epoch_progress_bar):
            ...
            with accelerator.accumulate(dit_model):
                try:
                    ...
                    # Sample x0 as Gaussian noise and x1 as the data latent
                    x0 = torch.randn_like(latents)  # x0 is the noise endpoint
                    x1 = latents  # x1 is the data endpoint
                    bsz = latents.shape[0]
                    
                    # Sample continuous time t from U[0, 1]
                    t_continuous = torch.rand((bsz,), device=latents.device, dtype=weight_dtype)
                    
                    # Create interpolated latent z_t = (1-t)*x0 + t*x1
                    # This is equivalent to: z_t = x0 + t*(x1 - x0)
                    t_expanded = t_continuous.reshape(-1, 1, 1, 1)  # Shape: [B, 1, 1, 1]
                    z_t = (1 - t_expanded) * x0 + t_expanded * x1  # Interpolated point
                    
                    # The target velocity is constant: v_target = x1 - x0
                    v_target = x1 - x0
                    
                    # Concatenate interpolated latents and conditioning latents along the channel dimension
                    concatenated_latents = torch.cat([z_t, conditioning_latents], dim=1)
                    concatenated_latents = concatenated_latents.to(dtype=z_t.dtype)

                    # --- DiT Model Prediction ---
                    # Pass the continuous time tensor directly to the model
                    # The model's internal forward method will handle the scaling from [0, 1] to [0, 999]
                    ...
                    model_pred = accelerator.unwrap_model(dit_model)(
                        ...
                        t=t_continuous,  # Use continuous time in [0, 1] range (model handles scaling internally)
                    ...
                    # For Rectified Flow Matching, the target is the velocity v_target = x1 - x0
                    ...
                    if accelerator.is_main_process and global_step % 100 == 0:
                        logger.debug(f"Step {global_step}: Using Rectified Flow Matching loss.")
