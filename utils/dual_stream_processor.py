...
def attention(query: torch.Tensor, key: torch.Tensor, value: torch.Tensor):
    try:
        ...
        # Fallback to vanilla attention if any error occurs
        query = einops.rearrange(query, 'b s h d -> b h s d')
        key = einops.rearrange(key, 'b s h d -> b h s d')
        value = einops.rearrange(value, 'b s h d -> b h s d')
...
class DualStreamAttnProcessor:
    """Attention processor used typically in processing the SD3-like self-attention projections."""

    def __call__(
        self,
        attn: "DualStreamAttention",
        ...
        **kwargs,
        ...
        query_i = attn.to_q(image_tokens).to(dtype=dtype)
        key_i = attn.to_k(image_tokens).to(dtype=dtype)
        ...
        if not attn.single:
            query_t = attn.to_q_t(text_tokens).to(dtype=dtype)
            key_t = attn.to_k_t(text_tokens).to(dtype=dtype)
